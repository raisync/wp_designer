...
(function($) {
        ...
        return $(this).each(function() {
            if ($(this).is('input') && $(this).attr('type') == "text") {
				if ( $(this).attr('max-year') ) { 
					var input_max_year = $(this).attr('max-year'); 
				} else { 
					var input_max_year = new Date().getFullYear(); 
				}
				
				if ( $(this).attr('min-year') ) { 
					var input_min_year = $(this).attr('min-year'); 
				} else { 
					var input_min_year = 1970; 
				}
				
				if ( $(this).attr('default-year') ) { 
					var input_default_year = $(this).attr('default-year'); 
				} else { 
					var input_default_year = new Date().getFullYear();
				}
				
				if ( $(this).attr('default-day') ) { 
					var input_default_day = $(this).attr('default-day'); 
				} else { 
					var input_default_day = new Date().getDate();
				}
				
				if ( $(this).attr('default-month') ) { 
					var input_default_month = $(this).attr('default-month')-1; 
				} else { 
					var input_default_month = new Date().getMonth();
				}
				
                var
                    t_y_cur = input_default_year,
                ...
                var
                    ...
                    _dd_settings = $.extend({
                        ...
                        lang: "en",
                        lock: false,
                        maxYear: input_max_year,
                        minYear: input_min_year,
                        yearsRange: 10,
                ...
                var
                    ...
                    _dd_submit = function() {
                        ...
                        if (_dd_settings.lock) {
                            ...
                            var
                                d1 = Date.parse(input_max_year + "-" + (t_m_cur + 1) + "-" + t_d_cur) / 1000,
                    ...
                    _dd_set = function() {
                        ...
                        if (_dd.hasClass('dd-init')) {
                            ...
                            _dd_m = input_default_month + 1,
							_dd_d = input_default_day,
							_dd_y = t_y_cur;
	...
	var lateYear = new Date().getFullYear()-6;
	var advanceYear = new Date().getFullYear()+2;
	$('.datedropper').dateDropper({
		init_animation: "dropdown", // fadein, bounce, dropdown
		format: "M j Y",
		minYear: lateYear,
		maxYear: advanceYear,
		yearsRange: 6,
		lock: "from",// false, "from", "to"
	});
