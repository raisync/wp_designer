	...
	for($i = 0; $i < count($settings->items); $i++) : if(!is_object($settings->items[$i])) continue;
		$output .= '<div class="slider-flickity-item '.$settings->items[$i]->slide_effect.' slider-item-'.($i+1).'">';
			...
			$output .= '<div class="slider-flickity-info">';
				$output .= '<div id="slider-flickity-info-'.$i.'" class="slider-flickity-info-inner'.$settings->items[$i]->content_aligment.$settings->items[$i]->content_aligment_horizontal.$mobile_centered.'">';
                    
					if ( !empty( $settings->items[$i]->slider_title ) ) {
						// $output .= '<h2 class="title">'.$settings->items[$i]->slider_title.'</h2>';
						$output .= '<h3 class="pretitle animation fl_appear no-desc">'.$settings->items[$i]->slider_title.'</h3>';
					}
					if ( !empty( $settings->items[$i]->slider_description ) ) {
						// $output .= '<p class="desc">'.$settings->items[$i]->slider_description.'</p>';
						$output .= '<h2 class="title animation fl_appear no-desc">'.$settings->items[$i]->slider_description.'</h2>';
					}
					...
					if ( !empty( $settings->items[$i]->btn_link ) || !empty( $settings->items[$i]->btn_link_secondary ) ) {
						...
						if ( !empty( $settings->items[$i]->btn_link ) ) {
							...
							$output .='<a class="slider-ui-button highlight btn-'.$i.' btn '.$btn_icon.$settings->items[$i]->btn_type.$settings->items[$i]->btn_icon_animation.$settings->items[$i]->btn_style.$settings->items[$i]->btn_bg_gradient_border.$settings->items[$i]->btn_effects.' '.$settings->btn_size.$settings->btn_corners.$settings->btn_width.'" data-toggle="modal" data-target="#inquireNowModal" href="'.$settings->items[$i]->btn_link.'" target="'.$settings->items[$i]->btn_link_target.'" role="button"'.$nofollow.'>';
								$output .=$icon_before.$settings->items[$i]->btn_text.$icon_after;
							$output .='</a>';
						}
