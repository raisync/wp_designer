...
?>
	<?php if( class_exists('acf') ) { 
		...
		/*background*/
		$pagefeaturedImg = wp_get_attachment_image_src( get_post_thumbnail_id($post->ID, $blogSetting), 'full' ); 
		$pagefeaturedImgURL = $pagefeaturedImg[0];
		$page_title_background = get_field('page_title_background', $blogSetting);
		if ($page_title_background <> '') {
			$page_title_bg = 'background-image: url('.$page_title_background['url'].');';
		} elseif ( $pagefeaturedImg ) {
			$page_title_bg = 'background-image: url('.$pagefeaturedImgURL.');';
		}
	...
	<section class="page-heading <?php echo $page_title_alignment_class; ?> parallax"<?php echo $page_heading_styles; ?>
		...
		data-top-bottom="background-position: <?php echo $page_title_background_position_parralax_end <> '' ? $page_title_background_position_parralax_end : '50% 70%'; ?>"
	>
